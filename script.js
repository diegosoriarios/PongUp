...
function Pong(appendToElementId, window, document) {
    ...
    function render() {
        ...
        if(powerUp !== undefined) {
            ...
            context.fillRect(powerUp.x, powerUp.y, powerUp.w, powerUp.h)
            context.fillStyle = powerUp.textColor
            context.rect(powerUp.x, powerUp.y, powerUp.w, powerUp.h)
            context.stroke()
            context.fillStyle = powerUp.textColor
            context.textAlign = "center"
            context.fillText(powerUp.text, powerUp.x + 20, powerUp.y + 20)
        }
    }
    ...
    function update() {
        ...
        time += 1/60
        if((Math.floor(time) % 8) === 0 && Math.floor(time) !== 0) {
            if(!hasPowerUp){
        ...
        if((Math.floor(time) % 5 === 0) && Math.floor(time) !== 0) {
            if(powerUpActive){
                ...
                computer.paddle.width = 50
            }
        }

        if(powerUp !== undefined) {
            if(ball.x - ball.radius < powerUp.x + powerUp.w && ball.y - ball.radius < powerUp.y + powerUp.h &&
               ball.x + ball.radius > powerUp.x && ball.y + ball.radius > powerUp.y) {
                   powerUpActive = true
                   time = 0
                   switch(powerUp.type) {
                       case 0:
                        ...
                        case 4:
                            ...
                            break
                        case 5:
                            computer.paddle.width = 100
                            break
                        case 6:
                            player.paddle.width = 25
                            break
                        case 7:
                            computer.paddle.width = 25
                            break
                   }
               }
        }
...
function generatePowerUp() {
    ...
    return {
        x: Math.floor(Math.random() * (WIDTH - 64)),
        y: HEIGHT / 2,
        type: type,
        w: 32,
        h: 32,
